# Exercício mal explicado! Enunciados mal explanados.
...
def calcula_assinatura(texto):
    ...
    for i in texto:
        sentencas = []
        sentencas = separa_sentencas(str(i))  # lista comum, ~matriz
        frases = []
        num_tot_sentencas = 0
        soma_cat_sentencas = 0
        for i in range(len(sentencas)):
            frase_i = separa_frases(str(sentencas[i]))
            frases.append(frase_i)  # matriz, lista de listas
            num_tot_sentencas += 1
        ...
        for lin in range(len(frases)):
            for col in range(len(frases[lin])):
                ...
                palavras.append(palavra_i)  # matriz, lista de listas
                num_tot_frases += 1
                soma_cat_frases = soma_cat_frases + len(str(frases[lin][col]))
        mtrx_para_lista = []  # palavras de matriz para lista
